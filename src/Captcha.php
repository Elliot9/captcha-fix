...
class Captcha
{
    ...
    public function create_audio($config = 'default')
    {
        ...
        $result=$client->ConvertText($this->account,$this->password,$this->text,"Theresa",100, 0, "wav");
        ...
        {
            sleep(2);
            $result=$client->GetConvertStatus($this->account,$this->password,$resultConvertID);
            $resultArray= explode("&",$result);
            list($resultCode, $resultString, $statusCode, $status, $resultUrl) = $resultArray;
            if($resultCode =='0' && $statusCode =='2')
            {
                //return "<audio src='$resultUrl' controls='controls'>wav</audio>";
                return $resultUrl;
            }
        }
    ...
    //0322
    public function audio_src($config = null)
    {
        return url('captcha/audio' . ($config ? '/' . $config : '/default')) . '?' . $this->str->random(8);
    }
}
